    ...
    <div className="max-w-2xl mx-auto">
      <div className="mb-6 flex justify-between items-end">
        <div>
            ...
            {error && <p className="text-red-500 text-sm mt-1 flex items-center"><AlertCircle size={12} className="mr-1"/> {error}</p>}
        </div>
          ...
          {q.options.map((option, index) => (
            ...
            >
              <div className="flex items-center justify-between">
                <span>{option}</span>
                {isAnswered && index === q.correct && <CheckCircle size={20} className="text-green-600" />}
              </div>
            </button>
          ))}
        </div>
